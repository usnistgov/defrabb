...
def process_exclusion(
    exclusion_path: str, dip_bed: BedTool, output_dir: str
) -> (str, int, float):
...
def accumulate_intersections(exclusion_filenames: List[str]) -> BedTool:
    ...
    for filename in exclusion_filenames[1:]:
        accumulated_intersections = accumulated_intersections.cat(
            filename, postmerge=True
        )
    return accumulated_intersections


def generate_summary_data(
    dip_bed_path: str, exclusion_paths: List[str], intersection_dir: str
) -> List[dict]:
    ...
    summary_data.append(
        {"genomic_region": "initial", "bp": total_dip_bp, "pct_of_initial": 0}
    )

    if exclusion_paths:
        processed_exclusions = [
            process_exclusion(path, dip_bed, intersection_dir)
        ...
        summary_data.append(
            {
                "genomic_region": "benchmark_regions",
                "bp": remaining_bp,
                "pct_of_initial": remaining_pct,
            }
        )
        summary_data.append(
            {
                "genomic_region": "total_excluded",
                "bp": total_excluded_bp,
                "pct_of_initial": total_excluded_pct,
            }
        )
    else:
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Calculate the excluded base pairs from a dip.bed file using exclusion bed files."
    )
    parser.add_argument("dip_bed_path", type=str, help="Path to the dip.bed file.")
    parser.add_argument(
        "summary_table_path",
        type=str,
        help="Path where the summary table will be saved.",
    )
    parser.add_argument(
        "intersection_dir",
        type=str,
        help="Directory to write dip.bed and exclusion intersection bed files.",
    )
    parser.add_argument(
        "exclusion_paths", type=str, nargs="+", help="Paths to the exclusion bed files."
    )
...
def main(
    ...
    summary_data = generate_summary_data(
        dip_bed_path, exclusion_paths, intersection_dir
    )
...
if __name__ == "__main__":
    ...
    main(
        args.dip_bed_path,
        args.summary_table_path,
        args.exclusion_paths,
        args.intersection_dir,
    )
