...
DT::datatable(
        ...
        extensions = "Buttons",
        options = list(
                pageLength = nrow(asm_var_rtg_stat_df),
                dom = 'Bfrtip',
    ...
    htmltools::tagList(
      exclusion_df %>%
        ...
        DT::datatable(
          ...
          rownames = FALSE,
          filter = 'top',
          extensions = list('Buttons', 'ColReorder','FixedColumns'),
          options = list(
            dom = 'Bfrtip',
            buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
            pageLength = nrow(.),
        ...
        DT::formatRound(
          columns = c("new_excluded_bp",
                      "resulting_length",
                      "dip_overlap_bp"),
          digits = 0
        ) %>%
        DT::formatPercentage(
          columns = c("dip_excluded_pct", "pct_of_initial"),
          digits = 1
        )
    )
  )
...
DT::datatable(
  ...
  options = list(
    dom = 'Bfrtip',
    buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
    pageLength = nrow(happy_summary_df),
    colReorder = TRUE,
    scrollX = TRUE,
    fixedColumns = list(leftColumns = 5)
  )
...
happy_extended_df <- run_params$inputs$happy_extended %>%
  set_names(str_extract(., "(?<=happy/).*(?=/)")) %>%
  map_dfr(
    load_happy_extended,
    filter = TRUE,
    subsets = subsets,
    metrics = metrics_to_keep,
    .id = "evaluation"
  ) %>%
  ## Assumes evaluation naming convention used in analysis tables
  mutate(
  ...
  select(ref, truth, query, target_regions, c(2:15))
...
DT::datatable(
  ...
  options = list(
    ...
    colReorder = TRUE,
    scrollX = TRUE,
    fixedColumns = list(leftColumns = 7)
  )
)
```
...
DT::datatable(
  ...
  options = list(
    dom = 'Bfrtip',
    buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
    pageLength = nrow(truvari_summary_df),
    colReorder = TRUE,
    fixedColumns = list(leftColumns = 5)
  )
) %>%
