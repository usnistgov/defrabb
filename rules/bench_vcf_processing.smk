...
rule fix_XY_genotype:
	input: "results/draft_benchmarksets/{bench_id}/intermediates/{prefix}.vcf.gz"
	output: "results/draft_benchmarksets/{bench_id}/intermediates/{prefix}.halfcalltohomvar.vcf.gz"
	group: "make_bench"
	conda: "../envs/dipcall.yml"
	log: "logs/fix_XY_genotype/{bench_id}_{prefix}.log"
	shell: """
	gunzip -c {input} \
		| sed 's/\.|1/1\/1/;s/1|\./1\/1/' \
		| bgzip -c \
...
rule dip_gap2homvarbutfiltered:
	input: "results/draft_benchmarksets/{bench_id}/intermediates/{prefix}.vcf.gz"
	output: "results/draft_benchmarksets/{bench_id}/intermediates/{prefix}.gap2homvarbutfiltered.vcf.gz"
	group: "make_bench"
    # bgzip is part of samtools, which is part of the dipcall env
	conda: "../envs/dipcall.yml"
	log: "logs/dip_gap2homvarbutfiltered/{bench_id}_{prefix}.log"
	shell: """
		gunzip -c {input} |\
		sed 's/1|\./1|1/' |\
		grep -v 'HET\|GAP1\|DIP' |\
...
rule split_multiallelic_sites:
    ...
    log: "logs/split_multiallelic_sites/{bench_id}_{prefix}.log",
    shell:
        """
...
rule move_asm_vcf_to_draft_bench:
    ...
    output: "results/draft_benchmarksets/{bench_id}/intermediates/{prefix}.vcf.gz",
    log: "logs/process_benchmark_vcf/{bench_id}_{prefix}.log",
    shell: "cp {input} {output} &> {log}"

def get_processed_vcf(wildcards):
	vcf_suffix=bench_tbl.loc[wildcards.bench_id, 'bench_vcf_processing']
	if vcf_suffix == "none":
		return f"results/draft_benchmarksets/{{bench_id}}/intermediates/{{prefix}}.vcf.gz"
	else:
		return f"results/draft_benchmarksets/{{bench_id}}/intermediates/{{prefix}}.{vcf_suffix}.vcf.gz"

rule move_processed_draft_bench_vcf:
	input: get_processed_vcf
	output: "results/draft_benchmarksets/{bench_id}/{prefix}.vcf.gz"
	log: "logs/move_processed_draft_bench_vcf/{bench_id}_{prefix}.log",
